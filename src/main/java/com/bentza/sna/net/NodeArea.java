...
public class NodeArea extends JButton implements MouseListener,
    ...
    private static NodeSettingsDialog node_dialog;
    ...
    public void setInitialSettings()
        ...
        try
            {
            title = MainFrame.getCurrentNetwork().getName(); //
            } catch (Exception e1)
            {
            title = M14;
            }
    ...
    public void setLagrangeLayout()
        ...
        boolean[][] mat = MainFrame.getCurrentNetwork().getBooleanMatrix();
    ...
    public boolean selectActor(int tmp_index)
        ...
        GrNet.setHTMLStatus(M01 + my_nodes[selected_node].name + M02
    ...
    public static void setDefaultNodeFaceSource(String tmp_face_source)
        {
        MainFrame.setDefaultNodeFaceSource(tmp_face_source);
        }
    ...
    public void paintArrow(int x1, int y1, int x2, int y2,
        ...
        if (print_edge_value && tg == edges_gr)
            ...
            tg.drawString(String.valueOf(MainFrame.getCurrentNetwork()
    ...
    public void mouseClicked(MouseEvent e)
        ...
        if (e.getClickCount() > 1)
            ...
            if (selnode >= 0)
                ...
                if (node_dialog == null)
                    ...
                    try
                        {
                        node_dialog = new NodeSettingsDialog(candidate, selnode);
      ...
      AgnaLog.warn("suppressed exception", e1);
                ...
                try
                    ...
                    int x = e.getX();
                    int y = e.getY();
                    GrNet.doAddNodeHere(x, y, getWidth());
                    try
                        {
      ...
      AgnaLog.warn("suppressed exception", exc);
         ...
         * GrNet.setHTMLStatus("<html><font size = 2
    ...
    public void mouseDragged(MouseEvent e)
        ...
        if (this.moving_node != -1)
            ...
            GrNet.setHTMLStatus(M01 + my_nodes[moving_node].name + M11
     ...
     * String.valueOf(MainFrame.getCurrentNetwork().getValue(second_selected,
    ...
    public void mouseMoved(MouseEvent e)
        ...
        if (where != -1)
            ...
            colored_node = where;
            GrNet.setHTMLStatus(M01 + my_nodes[where].name);
            repaint();
            } else
            {
            colored_node = -1;
                    ...
                    if (second_selected >= 0 && allow_ES) // edge selected
                        GrNet
                                .setHTMLStatus(M01
                                        ...
                                        + M10
                                        + M06
                                        + String.valueOf(MainFrame
                                                .getCurrentNetwork().getValue(
                                                        second_selected,
                                                        selected_node)));
                    else
                        // edge not selected
                        GrNet.setHTMLStatus(M01 + my_nodes[selected_node].name
                                + M12);
                    } else
                    GrNet.setDefaultStatus();
                repaint();
