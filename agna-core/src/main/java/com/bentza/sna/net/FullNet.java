...
import com.bentza.sna.core.AgnaRowModel;
...
import javax.swing.JTextPane;
    ...
    public String getAgna2DefaultSettings(NodeArea area)
        ...
        if (area == null)
            {
            if (AppRuntime.getWorkingDirectory() != null)
                filestr.append("Working Directory\t"
                        + AppRuntime.getWorkingDirectory() + lb);
            return filestr.toString();
            }
        ...
        if (AppRuntime.getDefaultNodeFaceSource() != null
                || !(AppRuntime.getDefaultNodeFaceSource()).equals("-"))
            filestr.append("Classic Toolbar Icons\t"
                    + (AppRuntime.isClassicToolbarIcons() ? "yes" : "no")
                    + lb);
            filestr.append("Look And Feel\t"
                    + AppRuntime.getLookAndFeel() + lb);
            filestr.append("Pajek Vectors Enabled\t"
                    + (AppRuntime.isPajekVectorsEnabled() ? "yes"
                            : "no") + lb);
            filestr.append("Default Export Format\t"
                    + String.valueOf(AppRuntime.getRememberedExportFormat())
                    + lb);
            filestr.append("Default Node Face\t"
                    + AppRuntime.getDefaultNodeFaceSource() + lb);
    ...
    public void readNetwork(String str, String extension)
        ...
        if (error_list != null)
            ...
            if (AppRuntime.getDialogParent() == null
                    || java.awt.GraphicsEnvironment.isHeadless())
                {
                AgnaLog.warn(report);
                } else
                {
                JOptionPane.showMessageDialog(AppRuntime.getDialogParent(),
                        report, "Error List",
    ...
    public static void parseAgnaNonGraphicDefaultSettings(String str)
        ...
        if (tmp_val != null && !tmp_val.equals("-"))
            AppRuntime.setDefaultNodeFaceSource(tmp_val);
        tmp_val = parseFindNextWord(str, "Working Directory");
        if (tmp_val != null && !tmp_val.equals("-"))
            AppRuntime.setWorkingDirectory(tmp_val);
        tmp_val = parseFindNextWord(str, "Classic Toolbar Icons");
        if (tmp_val != null)
            {
            AppRuntime.setClassicToolbarIcons(tmp_val.equals("yes"));
            }
        tmp_val = parseFindNextWord(str, "Look And Feel");
        if (tmp_val != null)
            {
            AppRuntime.setLookAndFeel(tmp_val);
            }
        tmp_val = parseFindNextWord(str, "Pajek Vectors Enabled");
        if (tmp_val != null)
            {
            AppRuntime.setPajekVectorsEnabled(tmp_val.equals("yes"));
            }
        ...
        if (tmp_val != null && !tmp_val.equals("-"))
            ...
            try
                {
                AppRuntime.setRememberedExportFormat(Integer.parseInt(tmp_val));
                } catch (NumberFormatException e)
                {
...
tmp_node.setFace(tmpname);
                ...
                if ("-".equals(tmp_node.getFaceSource()))
                    ...
                    if ("-".equals(tmp_node.getFaceSource()))
                        ...
                        tmp_node.setFace(AppRuntime.getDefaultNodeFaceSource());
                        if (!is_err)
                            {
                            errors
    ...
    public void removeOutsidersInNetwork(AgnaRowModel tmp_model)
        {
